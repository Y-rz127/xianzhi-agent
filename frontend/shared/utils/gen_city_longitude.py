...
用法：python frontend/shared/utils/gen_city_longitude.py
"""
import io
import os
import re

HERE = os.path.dirname(os.path.abspath(__file__))  # <root>/frontend/shared/utils
...
def main():
    ...
    for name, lon in pairs:
        ...
        if key not in seen:
            seen[key] = float(lon)
    lines = [
        '"""中国城市经度映射（由 frontend/shared/utils/region-data.ts 自动生成，勿手工编辑）。',
        "",
        "用于真太阳时校正：出生地城市名 -> 东经度数。",
        "区县继承所属城市经度（同市经度差对真太阳时无实质影响），故仅保留城市级。",
        "数据源变更后请重新运行 frontend/shared/utils/gen_city_longitude.py。",
        '"""',
        "",
