...
# 传统节日 → 农历（月, 日）中文表述，供出生时间解析与农历转换共用
FESTIVAL_LUNAR_DATES = {
    "春节": ("正", "初一"), "元旦": ("正", "初一"),
    "端午": ("五", "初五"), "端午日": ("五", "初五"),
    "中秋": ("八", "十五"), "中秋日": ("八", "十五"),
    "重阳": ("九", "初九"), "重阳节": ("九", "初九"),
...
def _parse_birth_smart(birth_time: str) -> tuple:
    ...
    for festival, (mo, day) in FESTIVAL_LUNAR_DATES.items():
...
def lunar_to_solar(query: str) -> str:
    ...
    try:
        ...
        FESTIVAL_MAP = {**FESTIVAL_LUNAR_DATES, "冬至": ("十一", "初")}  # 冬至按节气，下面特殊处理
        s = query.strip()
        # 提取年份
        ym = re.search(r"(\d{4})年", s)
        year = int(ym.group(1)) if ym else None
