"""基于先知图表的确定性工作流 Supervisor（XianzhiWorkflow）。

R9 拆分后，意图/模型/Worker 注册表/Reviewer 已拆至
...
本模块是 XianzhiWorkflow 的唯一归属（编排核心），不再作为门面重导出——
所有公共符号（build_chart_context / classify_question / QuestionIntent 等）
统一从其定义所在的子模块直接导入。
"""
...
class XianzhiWorkflow:
    ...
    def answer(
        ...
        # 故闲聊/分析一致开启思考。由 use_thinking 写入 contextvar，图内 generate/repair 节点自动读取。
