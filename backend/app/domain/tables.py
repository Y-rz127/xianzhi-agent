"""八字查表数据：五行/干支基础表、合冲刑害表、神煞查表。

R9 拆分自 bazi_engine.py，纯数据无逻辑；bazi_engine 门面重导出全部符号。"""

from __future__ import annotations

GAN_WUXING = {
    "甲": "木",
    "乙": "木",
    "丙": "火",
    "丁": "火",
    "戊": "土",
    "己": "土",
    "庚": "金",
    "辛": "金",
    "壬": "水",
    "癸": "水",
}


ZHI_WUXING = {
    "子": "水",
    "丑": "土",
    "寅": "木",
    "卯": "木",
    "辰": "土",
    "巳": "火",
    "午": "火",
    "未": "土",
    "申": "金",
    "酉": "金",
    "戌": "土",
    "亥": "水",
}


GZ_WUXING = {**GAN_WUXING, **ZHI_WUXING}


HIDDEN_STEMS = {
    "子": (("癸", 1.0),),
    "丑": (("己", 0.6), ("癸", 0.3), ("辛", 0.1)),
    "寅": (("甲", 0.6), ("丙", 0.3), ("戊", 0.1)),
    "卯": (("乙", 1.0),),
    "辰": (("戊", 0.6), ("乙", 0.3), ("癸", 0.1)),
    "巳": (("丙", 0.6), ("戊", 0.3), ("庚", 0.1)),
    "午": (("丁", 0.7), ("己", 0.3)),
    "未": (("己", 0.6), ("丁", 0.3), ("乙", 0.1)),
    "申": (("庚", 0.6), ("壬", 0.3), ("戊", 0.1)),
    "酉": (("辛", 1.0),),
    "戌": (("戊", 0.6), ("辛", 0.3), ("丁", 0.1)),
    "亥": (("壬", 0.7), ("甲", 0.3)),
}


GENERATES = {"木": "火", "火": "土", "土": "金", "金": "水", "水": "木"}


CONTROLS = {"木": "土", "土": "水", "水": "火", "火": "金", "金": "木"}


WUXING_ORDER = ("金", "木", "水", "火", "土")


CHANG_SHENG = ("长生", "沐浴", "冠带", "临官", "帝旺", "衰", "病", "死", "墓", "绝", "胎", "养")


_GAN_CHANGSHENG_ZHI = {
    "甲": "亥",
    "丙": "寅",
    "戊": "寅",
    "庚": "巳",
    "壬": "申",
    "乙": "午",
    "丁": "酉",
    "己": "酉",
    "辛": "子",
    "癸": "卯",
}


_YANG_GAN = {"甲", "丙", "戊", "庚", "壬"}


_ZHI_SEQ = ["子", "丑", "寅", "卯", "辰", "巳", "午", "未", "申", "酉", "戌", "亥"]


LIU_HE = {
    frozenset(("子", "丑")): "子丑合土",
    frozenset(("寅", "亥")): "寅亥合木",
    frozenset(("卯", "戌")): "卯戌合火",
    frozenset(("辰", "酉")): "辰酉合金",
    frozenset(("巳", "申")): "巳申合水",
    frozenset(("午", "未")): "午未合土",
}


GAN_HE = {
    frozenset(("甲", "己")): "甲己合土",
    frozenset(("乙", "庚")): "乙庚合金",
    frozenset(("丙", "辛")): "丙辛合水",
    frozenset(("丁", "壬")): "丁壬合木",
    frozenset(("戊", "癸")): "戊癸合火",
}


GAN_CHONG = {
    frozenset(("甲", "庚")): "甲庚冲",
    frozenset(("乙", "辛")): "乙辛冲",
    frozenset(("丙", "壬")): "丙壬冲",
    frozenset(("丁", "癸")): "丁癸冲",
}


LIU_CHONG = {
    frozenset(("子", "午")): "子午冲",
    frozenset(("丑", "未")): "丑未冲",
    frozenset(("寅", "申")): "寅申冲",
    frozenset(("卯", "酉")): "卯酉冲",
    frozenset(("辰", "戌")): "辰戌冲",
    frozenset(("巳", "亥")): "巳亥冲",
}


CHONG_ZHI = {
    "子": "午",
    "午": "子",
    "丑": "未",
    "未": "丑",
    "寅": "申",
    "申": "寅",
    "卯": "酉",
    "酉": "卯",
    "辰": "戌",
    "戌": "辰",
    "巳": "亥",
    "亥": "巳",
}


LIU_HAI = {
    frozenset(("子", "未")): "子未害",
    frozenset(("丑", "午")): "丑午害",
    frozenset(("寅", "巳")): "寅巳害",
    frozenset(("卯", "辰")): "卯辰害",
    frozenset(("申", "亥")): "申亥害",
    frozenset(("酉", "戌")): "酉戌害",
}


# 三刑：三支齐全为「真刑」，两支见为「半刑」。
# 知识库 `12_合冲刑害规则卡.md` §三 采用「两支半刑」口径（§三.4 明文：「本规则卡采用
# '两支半刑'口径」，并注明部分流派坚持三支齐全、实战可两参）。
#
# ⚠️ 标签方向曾写反（2026-09-15 修正）：原表把 寅巳申 标成「持势」、丑未戌 标成「无恩」，
# 而知识库 §三.1/§三.2 明确 寅巳申=无恩之刑、丑未戌=恃势之刑。属"字典抄错"类独立故障。
#
# 字段：(相刑循环序 a,b,c, 刑名, 显示序)。
#   循环序用于派生**半刑对**（丑刑戌、戌刑未、未刑丑 → 丑戌/戌未/未丑）；
#   显示序用于**真刑标签**（知识库 §三.2 标题为「丑未戌三刑」，取其惯用写法）。
SAN_XING_TRIPLE = (
    ("寅", "巳", "申", "无恩之刑", "寅巳申"),
    ("丑", "戌", "未", "恃势之刑", "丑未戌"),
)

# 天然两支的刑（无"三支齐全"之说，故不参与半刑派生）
SAN_XING_PAIR = {
    frozenset(("子", "卯")): "子卯无礼刑",
}

SAN_XING = {
    **{frozenset((a, b, c)): f"{disp}{name}三刑" for a, b, c, name, disp in SAN_XING_TRIPLE},
    **SAN_XING_PAIR,
}


SELF_XING = {"辰": "辰辰自刑", "午": "午午自刑", "酉": "酉酉自刑", "亥": "亥亥自刑"}


# 三合局（生-旺-墓）与所属五行。**中神为第 2 位** —— 半合（含中神的两支）与
# 拱合（首尾两支、中神虚一）都依赖它，故保留结构式定义；SAN_HE 由它派生。
# 这是全仓唯一的三合定义（`xipan._SAN_HE_GROUPS` 曾是一份完全相同的副本，已删除）。
SAN_HE_TRIPLE = (
    ("申", "子", "辰", "水"),
    ("亥", "卯", "未", "木"),
    ("寅", "午", "戌", "火"),
    ("巳", "酉", "丑", "金"),
)

SAN_HE = {frozenset((a, b, c)): f"{a}{b}{c}合{wx}局" for a, b, c, wx in SAN_HE_TRIPLE}


SAN_HUI = {
    frozenset(("寅", "卯", "辰")): "寅卯辰会东方木",
    frozenset(("巳", "午", "未")): "巳午未会南方火",
    frozenset(("申", "酉", "戌")): "申酉戌会西方金",
    frozenset(("亥", "子", "丑")): "亥子丑会北方水",
}


LIU_PO = {
    frozenset(("子", "酉")): "子酉破",
    frozenset(("卯", "午")): "卯午破",
    frozenset(("辰", "丑")): "辰丑破",
    frozenset(("巳", "申")): "巳申破",
    frozenset(("寅", "亥")): "寅亥破",
    frozenset(("未", "戌")): "未戌破",
}


TIAN_YI = {
    "甲": ("丑", "未"),
    "戊": ("丑", "未"),
    "庚": ("丑", "未"),
    "乙": ("子", "申"),
    "己": ("子", "申"),
    "丙": ("亥", "酉"),
    "丁": ("亥", "酉"),
    "壬": ("卯", "巳"),
    "癸": ("卯", "巳"),
    "辛": ("午", "寅"),
}


TAI_JI = {
    "甲": ("子", "午"),
    "乙": ("子", "午"),
    "丙": ("酉", "卯"),
    "丁": ("酉", "卯"),
    "戊": ("辰", "戌", "丑", "未"),
    "己": ("辰", "戌", "丑", "未"),
    "庚": ("寅", "亥"),
    "辛": ("寅", "亥"),
    "壬": ("巳", "申"),
    "癸": ("巳", "申"),
}


WEN_CHANG = {
    "甲": "巳",
    "乙": "午",
    "丙": "申",
    "丁": "酉",
    "戊": "申",
    "己": "酉",
    "庚": "亥",
    "辛": "子",
    "壬": "寅",
    "癸": "卯",
}


YANG_REN = {
    "甲": "卯",
    "乙": "寅",  # 乙羊刃有争议，多数取寅
    "丙": "午",
    "丁": "巳",
    "戊": "午",
    "己": "巳",
    "庚": "酉",
    "辛": "申",
    "壬": "子",
    "癸": "亥",
}


HUA_GAI = {
    "寅": "戌",
    "午": "戌",
    "戌": "戌",
    "巳": "丑",
    "酉": "丑",
    "丑": "丑",
    "申": "辰",
    "子": "辰",
    "辰": "辰",
    "亥": "未",
    "卯": "未",
    "未": "未",
}


TAO_HUA = {
    "寅": "卯",
    "午": "卯",
    "戌": "卯",
    "巳": "午",
    "酉": "午",
    "丑": "午",
    "申": "酉",
    "子": "酉",
    "辰": "酉",
    "亥": "子",
    "卯": "子",
    "未": "子",
}


YI_MA = {
    "寅": "申",
    "午": "申",
    "戌": "申",
    "巳": "亥",
    "酉": "亥",
    "丑": "亥",
    "申": "寅",
    "子": "寅",
    "辰": "寅",
    "亥": "巳",
    "卯": "巳",
    "未": "巳",
}


JIANG_XING = {
    "寅": "午",
    "午": "午",
    "戌": "午",
    "巳": "酉",
    "酉": "酉",
    "丑": "酉",
    "申": "子",
    "子": "子",
    "辰": "子",
    "亥": "卯",
    "卯": "卯",
    "未": "卯",
}


LU_SHEN = {
    "甲": "寅",
    "乙": "卯",
    "丙": "巳",
    "丁": "午",
    "戊": "巳",
    "己": "午",
    "庚": "申",
    "辛": "酉",
    "壬": "亥",
    "癸": "子",
}


TIAN_DE_MONTH = {
    1: ("丁",),  # 正月(寅月)：见丁
    2: ("申",),  # 二月(卯月)：见申（地支！）
    3: ("壬",),  # 三月(辰月)：见壬
    4: ("辛",),  # 四月(巳月)：见辛
    5: ("亥",),  # 五月(午月)：见亥（地支！）
    6: ("甲",),  # 六月(未月)：见甲
    7: ("癸",),  # 七月(申月)：见癸
    8: ("寅",),  # 八月(酉月)：见寅（地支！）
    9: ("丙",),  # 九月(戌月)：见丙
    10: ("乙",),  # 十月(亥月)：见乙
    11: ("巳",),  # 十一月(子月)：见巳（地支！）
    12: ("庚",),  # 十二月(丑月)：见庚
}


TIAN_DE_IS_BRANCH = {2, 5, 8, 11}  # 这几个月的天德是地支而非天干


YUE_DE_MONTH = {
    1: ("丙",),
    2: ("甲",),
    3: ("壬",),
    4: ("庚",),
    5: ("丙",),
    6: ("甲",),
    7: ("壬",),
    8: ("庚",),
    9: ("丙",),
    10: ("甲",),
    11: ("壬",),
    12: ("庚",),
}


SHI_E_DA_BAI = (
    "甲辰",
    "乙巳",  # ← 原：乙亥（错）
    "丙申",
    "丁亥",
    "戊戌",  # ← 原：戊午（错）
    "己丑",  # ← 原：己巳（错）
    "庚辰",  # ← 原：庚午（错）
    "辛巳",
    "壬申",
    "癸亥",
)


XUE_TANG = {"金": "巳", "木": "亥", "水": "申", "土": "申", "火": "寅"}


CI_GUAN = {"金": "申", "木": "寅", "水": "亥", "土": "亥", "火": "巳"}


ZHENG_XUE_TANG = {"金": "辛巳", "木": "己亥", "水": "甲申", "土": "戊申", "火": "丙寅"}


ZHENG_CI_GUAN = {"金": "壬申", "木": "庚寅", "水": "癸亥", "土": "丁亥", "火": "乙巳"}


JIN_YU = {
    "甲": "辰",
    "乙": "巳",
    "丙": "未",
    "丁": "申",
    "戊": "未",
    "己": "申",
    "庚": "戌",
    "辛": "亥",
    "壬": "丑",
    "癸": "寅",
}


FU_XING = {
    "甲": ("寅", "子"),
    "丙": ("寅", "子"),
    "乙": ("卯", "丑"),
    "癸": ("卯", "丑"),
    "戊": ("申",),
    "己": ("未",),
    "丁": ("亥",),
    "庚": ("午",),
    "辛": ("巳",),
    "壬": ("辰",),
}


JIE_SHA = {
    # 水局(申子辰)：木绝于巳
    "申": "巳",
    "子": "巳",
    "辰": "巳",
    # 火局(寅午戌)：火绝于亥
    "寅": "亥",
    "午": "亥",
    "戌": "亥",
    # 木局(亥卯未)：水绝于申
    "亥": "申",
    "卯": "申",
    "未": "申",
    # 金局(巳酉丑)：土绝于寅
    "巳": "寅",
    "酉": "寅",
    "丑": "寅",
}


ZAI_SHA = {
    # 水(申子辰)→将星子→冲午
    "申": "午",
    "子": "午",
    "辰": "午",
    # 火(寅午戌)→将星午→冲子
    "寅": "子",
    "午": "子",
    "戌": "子",
    # 木(亥卯未)→将星卯→冲酉
    "亥": "酉",
    "卯": "酉",
    "未": "酉",
    # 金(巳酉丑)→将星酉→冲卯
    "巳": "卯",
    "酉": "卯",
    "丑": "卯",
}


WANG_SHEN = {
    # 火局(寅午戌)：火临官在巳
    "寅": "巳",
    "午": "巳",
    "戌": "巳",
    # 木局(亥卯未)：木临官在寅
    "亥": "寅",
    "卯": "寅",
    "未": "寅",
    # 金局(巳酉丑)：金临官在申
    "巳": "申",
    "酉": "申",
    "丑": "申",
    # 水局(申子辰)：水临官在亥
    "申": "亥",
    "子": "亥",
    "辰": "亥",
}


DIAO_KE = {
    "子": "戌",
    "丑": "亥",
    "寅": "子",
    "卯": "丑",
    "辰": "寅",
    "巳": "卯",
    "午": "辰",
    "未": "巳",
    "申": "午",
    "酉": "未",
    "戌": "申",
    "亥": "酉",
}


SANG_MEN = {
    "子": "寅",
    "丑": "卯",
    "寅": "辰",
    "卯": "巳",
    "辰": "午",
    "巳": "未",
    "午": "申",
    "未": "酉",
    "申": "戌",
    "酉": "亥",
    "戌": "子",
    "亥": "丑",
}


BING_FU = {
    "子": "亥",
    "丑": "子",
    "寅": "丑",
    "卯": "寅",
    "辰": "卯",
    "巳": "辰",
    "午": "巳",
    "未": "午",
    "申": "未",
    "酉": "申",
    "戌": "酉",
    "亥": "戌",
}


TIAN_YI_MED = {
    "寅": "丑",
    "卯": "寅",
    "辰": "卯",
    "巳": "辰",
    "午": "巳",
    "未": "午",
    "申": "未",
    "酉": "申",
    "戌": "酉",
    "亥": "戌",
    "子": "亥",
    "丑": "子",
}


HONG_LUAN = {
    "亥": "辰",
    "卯": "子",
    "未": "申",  # 亥卯未
    "寅": "丑",
    "午": "酉",
    "戌": "巳",  # 寅午戌
    "申": "未",
    "子": "卯",
    "辰": "亥",  # 申子辰
    "巳": "戌",
    "酉": "午",
    "丑": "寅",  # 巳酉丑
}


TIAN_XI = {
    "亥": "戌",
    "卯": "午",
    "未": "寅",  # 亥卯未（红鸾对宫）
    "寅": "未",
    "午": "卯",
    "戌": "亥",  # 寅午戌
    "申": "丑",
    "子": "酉",
    "辰": "巳",  # 申子辰
    "巳": "辰",
    "酉": "子",
    "丑": "申",  # 巳酉丑
}


GU_CHEN = {
    "寅": "巳",
    "卯": "巳",
    "辰": "巳",  # 寅卯辰年 → 巳
    "巳": "申",
    "午": "申",
    "未": "申",  # 巳午未年 → 申
    "申": "亥",
    "酉": "亥",
    "戌": "亥",  # 申酉戌年 → 亥
    "亥": "寅",
    "子": "寅",
    "丑": "寅",  # 亥子丑年 → 寅
}


GUA_SU = {
    "寅": "丑",
    "卯": "丑",
    "辰": "丑",  # 寅卯辰年 → 丑
    "巳": "辰",
    "午": "辰",
    "未": "辰",  # 巳午未年 → 辰
    "申": "未",
    "酉": "未",
    "戌": "未",  # 申酉戌年 → 未
    "亥": "戌",
    "子": "戌",
    "丑": "戌",  # 亥子丑年 → 戌
}


BRANCH_ORDER = ["子", "丑", "寅", "卯", "辰", "巳", "午", "未", "申", "酉", "戌", "亥"]


SEASON_OF_BRANCH = {  # 月支对应季节
    "寅": "春",
    "卯": "春",
    "辰": "春",
    "巳": "夏",
    "午": "夏",
    "未": "夏",
    "申": "秋",
    "酉": "秋",
    "戌": "秋",
    "亥": "冬",
    "子": "冬",
    "丑": "冬",
}


GUO_YIN = {
    "甲": "戌",
    "乙": "亥",
    "丙": "丑",
    "丁": "寅",
    "戊": "丑",
    "己": "寅",
    "庚": "辰",
    "辛": "巳",
    "壬": "未",
    "癸": "申",
}


DE_XIU = {
    "寅": ({"丙", "丁"}, {"戊", "癸"}),
    "午": ({"丙", "丁"}, {"戊", "癸"}),
    "戌": ({"丙", "丁"}, {"戊", "癸"}),
    "申": ({"壬", "癸", "戊", "己"}, {"丙", "辛", "甲", "己"}),
    "子": ({"壬", "癸", "戊", "己"}, {"丙", "辛", "甲", "己"}),
    "辰": ({"壬", "癸", "戊", "己"}, {"丙", "辛", "甲", "己"}),
    "巳": ({"庚", "辛"}, {"乙", "庚"}),
    "酉": ({"庚", "辛"}, {"乙", "庚"}),
    "丑": ({"庚", "辛"}, {"乙", "庚"}),
    "亥": ({"甲", "乙"}, {"丁", "壬"}),
    "卯": ({"甲", "乙"}, {"丁", "壬"}),
    "未": ({"甲", "乙"}, {"丁", "壬"}),
}


YUE_DE_HE = {
    "寅": "辛",
    "午": "辛",
    "戌": "辛",
    "申": "丁",
    "子": "丁",
    "辰": "丁",
    "巳": "乙",
    "酉": "乙",
    "丑": "乙",
    "亥": "己",
    "卯": "己",
    "未": "己",
}


TIAN_DE_HE = {
    "寅": "壬",
    "卯": "巳",
    "辰": "丁",
    "巳": "丙",
    "午": "寅",
    "未": "己",
    "申": "戊",
    "酉": "亥",
    "戌": "辛",
    "亥": "庚",
    "子": "申",
    "丑": "乙",
}


SAN_QI = (("甲", "戊", "庚"), ("乙", "丙", "丁"), ("壬", "癸", "辛"))


TIAN_SHE = {"春": "戊寅", "夏": "甲午", "秋": "戊申", "冬": "甲子"}


LIU_XIU = {"丙午", "丁未", "戊子", "戊午", "己丑", "己未"}


TIAN_CHU = {
    "甲": "巳",
    "乙": "午",
    "丙": "巳",
    "丁": "午",
    "戊": "申",
    "己": "酉",
    "庚": "亥",
    "辛": "子",
    "壬": "寅",
    "癸": "卯",
}


GONG_LU = (
    ("亥", "丑", "子"),
    ("丑", "亥", "子"),
    ("巳", "未", "午"),
    ("未", "巳", "午"),
    ("辰", "午", "巳"),
)


SHI_LING = {"甲辰", "乙亥", "丙辰", "丁酉", "戊午", "庚戌", "庚寅", "辛亥", "壬寅", "癸未"}


TIAN_DI_ZHUAN = {
    "春": ("乙卯", "辛卯"),
    "夏": ("丙午", "戊午"),
    "秋": ("辛酉", "癸酉"),
    "冬": ("壬子", "丙子"),
}


KUI_GANG = {"庚辰", "壬辰", "戊戌", "庚戌"}


JIN_SHEN = {"乙丑", "己巳", "癸酉"}


BA_ZHUAN = {"甲寅", "乙卯", "丁未", "戊戌", "己未", "庚申", "辛酉", "癸丑"}


XUE_REN = {
    "寅": "丑",
    "卯": "未",
    "辰": "寅",
    "巳": "申",
    "午": "卯",
    "未": "酉",
    "申": "辰",
    "酉": "戌",
    "戌": "巳",
    "亥": "亥",
    "子": "午",
    "丑": "子",
}


PI_MA = {
    "子": "酉",
    "丑": "戌",
    "寅": "亥",
    "卯": "子",
    "辰": "丑",
    "巳": "寅",
    "午": "卯",
    "未": "辰",
    "申": "巳",
    "酉": "午",
    "戌": "未",
    "亥": "申",
}


TIAN_LUO = ("戌", "亥")


DI_WANG = ("辰", "巳")


JIU_CHOU = {"丁酉", "戊子", "戊午", "己卯", "己酉", "辛卯", "辛酉", "壬子", "壬午"}


SI_FEI = {
    "春": ("庚申", "辛酉"),
    "夏": ("壬子", "癸亥"),
    "秋": ("甲寅", "乙卯"),
    "冬": ("丙午", "丁巳"),
}


YIN_CHA_YANG_CUO = {
    "丙子",
    "丙午",
    "丁丑",
    "丁未",
    "戊寅",
    "戊申",
    "辛卯",
    "辛酉",
    "壬辰",
    "壬戌",
    "癸巳",
    "癸亥",
}


GU_LUAN = {"甲寅", "乙巳", "丙午", "丁巳", "戊午", "戊申", "辛亥", "壬子"}


LIU_XIA = {
    "甲": "酉",
    "乙": "戌",
    "丙": "未",
    "丁": "申",
    "戊": "巳",
    "己": "午",
    "庚": "辰",
    "辛": "卯",
    "壬": "亥",
    "癸": "寅",
}


HONG_YAN = {
    "甲": "午",
    "乙": "午",
    "丙": "寅",
    "丁": "未",
    "戊": "辰",
    "己": "辰",
    "庚": "戌",
    "辛": "酉",
    "壬": "子",
    "癸": "申",
}


SEASON_NOTES = {
    "寅": "春初木旺，重在疏土培木，兼看火来通明。",
    "卯": "仲春木旺，木气纯粹，宜看金土是否成器。",
    "辰": "季春湿土，木余水藏，需分清湿土与木气。",
    "巳": "初夏火旺，燥热渐起，喜水调候、金水相济。",
    "午": "仲夏火极，最怕燥烈失衡，调候优先看水。",
    "未": "季夏燥土，火土偏重时需金水润局。",
    "申": "初秋金旺，金气肃杀，宜看火炼与水润。",
    "酉": "仲秋金旺，金清则贵，过寒则需火暖。",
    "戌": "季秋燥土，火库土燥，喜水润燥。",
    "亥": "初冬水旺，寒气渐重，喜火暖局、土来制水。",
    "子": "仲冬水极，寒湿明显，调候首看火。",
    "丑": "季冬寒土，湿寒并见，喜火温土。",
}
