...
| 段落 | 最少 | 典型 | 上限 | 说明 |
|---|---|---|---|---|
| System Prompt | 2090 | 2090 | 2090 | `REVIEWER_SYSTEM`（prompts.py:362，实测 2090 字：逻辑自洽/断法/知识一致/古籍真实性/表达/事实/十神/神煞/宽容度/范围边界/岁运关系一致性/生克断言有据 12 维 + JSON 输出格式；十神维度含"天干地支分述不算混淆"等防误判口径） |
...
| **合计字数** | **~5446** | **~7149** | **~12649** | |
| **合计 Token** | **~9803** | **~12868** | **~22768** | |

> 上表为 2026-09-11 追加 Reviewer 维度 12（含十神维度防误判口径，System Prompt 1241→2090）后的重算值：其余段落实测值未变，仅在合计上叠加 +849 字（≈+1529 token）。
