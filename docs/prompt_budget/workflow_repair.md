...
| 段落 | 最少 | 典型 | 上限 | 说明 |
|---|---|---|---|---|
| System Prompt | 2359 | 2359 | 2359 | `REVIEWER_SYSTEM`（prompts.py:362，实测 2359 字：逻辑自洽/断法/知识一致/古籍真实性/表达/事实/十神/神煞/宽容度/范围边界/岁运关系一致性/生克断言有据 12 维 + JSON 输出格式；十神含"天干地支分述不算混淆"，事实含"事实窗口外不判错"，古籍含"检索片段内的引文句级复核"） |
...
| **合计字数** | **~5715** | **~7418** | **~12918** | |
| **合计 Token** | **~10287** | **~13352** | **~23252** | |

> 上表为 2026-09-13 Reviewer 防误判/防静默失效修复后的重算值：System Prompt 2090→2359（+269 字，≈+484 token），其余段落实测值未变，仅在合计上叠加该差值。
> 另注：修复后的"修复稿二次判定"不再调用 `review()`（会被正则短路），而是 `review_llm_only()`；
> 2026-09-13 二次收紧后，**修复稿无论 regex 是否通过都会跑一次 D**（regex 通过时用于复核改写质量，
> regex 仍报问题时用于判定是否正则误报）——即每次调用 C 之后固定多花 1 次调用 D。
> 修复轮次本身很少（观测期数天 3 次），因此增量成本可忽略；换来的是修复稿不再"regex 过即定稿"。
