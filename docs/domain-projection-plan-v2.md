# 让先知「按领域看全盘、一步步推演」——领域投影 + 岁运关系 + 分析规程（v2 定稿）

> v2 = 桌面稿（toasty-forging-curie-dwTUfht0.md）经代码核实后的修订定稿。本文自包含，无需对照 v1。

## 〇、v2 修订记录（相对 v1 的全部改动）

| # | 决策 | 内容 | 来源 |
| --- | --- | --- | --- |
| 1 | 大运统一 12 步 | 初始挂盘 10→12、db 10→12、工具 bazi_dayun 默认 8→12；cases.py 命例库保持 8（刻意小盘，不参与统一） | 用户拍板 |
| 2 | 年龄区间跨步 | 「30-40」与区间**有交集的大运全部命中**（0-2 个），`resolve_target_dayuns` 返回列表 | 用户拍板 |
| 3 | 补流月 | 目标流年附 12 节气月干支+十神一行（仅用户明确问到的年份、≤2 年）；不做流月×原局关系 | 用户拍板 |
| 4 | frozen intent 零替换 | v1 Step 8「把 target_years 并入大运区间」改为**纯函数派生**：`effective_target_years()` 四处调用，intent 在 classify 后永不变更，LangGraph state/节点零改动 | 评审裁决 |
| 5 | Reviewer 同源 | 新增 `build_sui_section(chart, intent)`，generate 与 check/repair 调**同一函数**，字节级一致 | 评审裁决 |
| 6 | 长度预算补全 | 岁运关系段 ≤300 字（单年）/ ≤600 字（大运）；fact_block 流年行 ≤12 行；流月 ≤2 行 | 评审裁决 |
| 7 | 砍私有重导出 | xipan 私有函数不进 bazi_engine 门面，`yun_relations` 直接 `from app.domain.xipan import ...`；门面只重导出新模块公共符号 | 评审裁决 |
| 8 | 正则层不校岁运 | `_regex_review`/`check_facts` 均不加岁运关系/流月干支校验，仅 LLM 维度 11 | 评审裁决 |
| 9 | 事实修正 | WORKERS 实为 **18 个**（v1 写 17）；`DayunItem.index` 透传 lunar `getIndex()`，基数实现时核对（见 Step 1 注意） | 核实 |
...
| 领域 Worker | `agent/workflow/workflow_workers.py` `WORKERS` | **18 个**领域，各有 expertise_prompt |
...
class SuiRelations:
    ...
    hits_yongshen: str         # 该岁运五行命中喜/忌/闲（解析 useful_hint 文本粗分类）
...
**`resolve_target_dayuns` 的 spec 语义（v2 定稿）：**
...
**注意（实现时核对）**：`DayunItem.index` 直接透传 lunar `getIndex()`——lunar 惯例 0=起运前、1=第一步大运。动工前先用真实盘打印 `[(d.index, d.ganzhi, d.start_year) for d in chart.dayun]` 核对基数；序号语义以「用户口语第一步 = 第一步大运」为准，必要时按「非空干支的第 N 个」对齐。
...
映射表（18 个 domain 全覆盖；实现时**神煞名必须与 `workflow_messages.py::_SHENSHA_NAMES` 逐字一致**，否则筛不出；性别从 `chart.birth.gender` 取，**无需加参数**）：
...
DOMAIN_STEP_FOCUS: dict[str, str] = {"career": "官杀(职位/权柄/压力)、印星(平台/靠山)、食伤(技能/表达) 三者的落点与流通", ...}
...
`REVIEWER_SYSTEM` 追加**维度 11「岁运关系一致性」**：回答若描述某年/某步大运与大运/原局构成某关系（天克地冲、岁运并临、伏吟、反吟、合冲刑害），须与【岁运关系】段一致；矛盾记问题；回答未描述关系则不记问题（宁可不误杀）。**流月干支本次不列入校验**（冒烟观察误报率后另议）。不动维度 9 白名单。
...
2. 新增 `build_sui_section(chart: BaziChart, intent: QuestionIntent) -> str`（**agent 层**，generate 与 Reviewer 的唯一同源函数）：
   ...
   ```
   `format_sui_relations` 压缩规则：**每个岁柱一行**（大运行带起止年，流年行带年份干支十神 + 关系短语 + hits_yongshen），确保大运场景 ≤600 字、单年场景 ≤300 字；超限从非目标年侧截断。
3. `compact_facts`：不改签名；在 `fact_block`（+合婚对方盘）之后追加
   ...
   ```
   仅当 `intent.needs_chart and not worker.skip_facts` 且 domain 有映射时注入（chitchat/theory 天然 skip_facts=True，零开销回归不变）。
4. `build_messages`：`system` 追加 `DOMAIN_PROCEDURE_TEMPLATE.format(领域=worker.label, 聚焦点=DOMAIN_STEP_FOCUS.get(domain, ""))`；`human` 顺序保持「用户问题 → 识别意图 → 历史摘要 → 系统排盘事实 → 本领域盘面要素 → 岁运关系 → 合婚基础/历史断事 → 命理规则检索 → 输出要求」。
5. `build_repair_messages`：同样带上「本领域盘面要素 + 岁运关系 + 分析规程」（**容易漏的一处**，进单测）。
...
- `bazi_liunian(...)` → 新增 `start_year: int | None = None`（现只能从当前年起算，问过去某年不支持），并附岁运关系。
...
- `backend/tests/test_domain_brief.py`：career 简报含正官/印星/月柱宫/将星；女命 love 用官杀、男命用财星（性别走 `chart.birth.gender`）；`theory/chitchat/auspicious` 返回空串；**神煞名与 `_SHENSHA_NAMES` 交叉校验**（防拼写漂移筛不出）。
...
| `build_repair_messages` 漏改导致修复后退化 | Step 9 第 5 点单列，进单测 |
| 神煞名与 `_SHENSHA_NAMES` 不一致筛空 | 单测交叉校验 |
...
- `backend/app/agent/prompts.py`（`DOMAIN_STEP_FOCUS` / `DOMAIN_PROCEDURE_TEMPLATE` / `domain_sysprompt` / `REVIEWER_SYSTEM` 维度 11）
...
- `backend/app/agent/workflow/workflow_messages.py`（主改动：fact_block 流年选择 / compact_facts 注入 / build_sui_section）
- `backend/app/agent/workflow/workflow_workers.py`（Reviewer `sui_text` 参数）
- `backend/app/agent/xianzhi_langgraph.py`（check_node / repair_node 传 `sui_text`）
- `backend/app/tools/bazi.py`（4 工具增强 + `bazi_dayun` 默认 12）
