...
| 校验维度 | 实现 | 失败示例 |
|---------|------|---------|
| **事实校验** | 复用 `check_facts`（四柱干支/流年干支/大运干支/十神存在性/神煞存在性与柱位归属）；合婚双盘时任一合法盘命中即放行 | "2024年甲辰" 写成 "2024年乙巳"；编造"丁卯大运"；排盘无七杀却说"七杀攻身"；年柱神煞说成日柱 |
| **古籍真实性** | `ANCIENT_CITATION_RE` 抽取「《XXX》原文/云/曰…」标注：① 书名须在检索结果或白名单 `{渊海子平, 子平真诠, 滴天髓, 穷通宝鉴, 三命通会, 神峰通考, 千里命稿}` 中出现；② 该书确在本次检索片段中时，引号原句也须能在片段里找到（标点差异不计） | 检索结果无《XX书》却引用；书名在检索中但引文是编造的 |
...
**干支/大运校验口径**（2026-09-13 修）：

1. **流年干支**：回答里"年份 + 干支"必须与该年真实流年干支一致。旧版把 `大运/运/走/交` 等词丢进
   ±30 字窗口判断，回答只要出现"运"字就整体跳过校验（实测"2030年是己酉年，这一步运里注意变动"
   漏检）。现在豁免需同时满足：该干支确实是本盘某步大运的干支，**且** "年份→干支"之间（≤6 字）
   或干支之后紧随大运词（"2028年走壬申大运"）。
2. **大运干支**：`(干支)(大运|运)` 必须是本盘（含合婚对方盘）真实存在的大运干支。流年干支只在
   "年份/流年紧邻干支"时才放行（"2032壬子运""流年壬子运"属近义写法，年份需贴在干支前 ≤8 字内）；
   无年份语境的"你走到己酉运"视为编造大运直接记问题——旧版无条件放行流年干支，编造大运只要
   撞上某个流年干支（60 甲子中约占窗口年份数）就拦不住。

**否定判定口径**（2026-09-13 修）：多字否定词（没有/不带/未见/并非…）直接命中；单字
"不/未/无/没/非/否"只在后接谓语（是/带/含/见/有…）或紧邻目标词时才算否定。旧版把单字当否定词，
"不错""无论""非常"会把肯定断言判成否定而放行。

**归属断言的岁运隔离**（2026-09-13 修）：锚点路径与目标词之间若插入了年份/大运/流年
（"你日柱壬申，2045年拱禄入命"），该目标词绑的是岁运而非原局柱，不算原局归属断言、不记问题。

**神煞柱位归属的配对口径**（2026-09-11 修：旧版"同一句里出现柱名 + 神煞名即判错"会造成系统性误报，
实测"时柱华盖、日柱学堂"与"…你月柱午中丁火正财，流年还带红艳煞"两处正确表述都被打回并触发无效修复）：

1. **按小句配对**：以 `。；;！!？?` 换行 与 `，,、` 切分，只在同一个小句内认定归属。
   "时柱华盖、日柱学堂" → 两个小句各含一个柱名、一个神煞，互不串味；
   "…你月柱午中丁火正财，流年还带红艳煞" → 后半句无柱名，不构成归属断言。
2. **就近取柱**：小句内多个柱名时取离神煞最近者；最近距离并列的柱名只要有一个合法即放行。
3. **纯岁运神煞不按原局柱位判错**：原局四柱没有、只在大运/流年（`chart.dayun/liunian[].shensha`）
   出现的神煞（如 2032 壬子的红艳煞），只有回答在小句里把它绑到某原局柱、且该小句无岁运语境时才记问题；
   其存在性仍由神煞存在性校验兜底。
4. **子串消歧**：神煞名之间存在包含关系（`学堂`⊂`正学堂`、`词馆`⊂`正词馆`），校验前把更长名就地
   替换为等长圆点（长度不变，位置/小句边界可直接复用），避免"正学堂"被当成"学堂"判柱位。

**第 2 层：LLM 深审（1 次调用）**——正则全通过后，用
`prompts.REVIEWER_SYSTEM`（实测 2090 字）做深度审核，维度覆盖：
逻辑自洽、断法准确性、知识一致性、古籍真实性（识别检索未提供的**伪造古文句子**，
正则只能查书名）、表达质量、事实复查（含"事实窗口外不判错"：流年/大运只是本次注入的部分范围）、
十神事实复查（含"天干/地支分述不算混淆""先查副星字段"、
...
**解析与降级口径**（2026-09-13 修，此前存在"静默失效"通道）：

- `pass` 字段做类型归一化：字符串 `"false"/"否"/"不通过"` 视为不通过。旧版 `bool("false")`
  为真，模型明确判不通过会被当成通过。
- JSON 解析改为**大括号平衡扫描**（逐个大括号起点尝试，优先取最后一个含 `pass` 的对象）：
  旧版用贪婪正则「第一个 `{` 到最后一个 `}`」，只要思考文字里出现大括号就整体解析失败。
- 审核调用走 `invoke_review()`（与生成路径同样 `bind(timeout=...)` + `clean_think_tags`），
  不再直接调 `chat_model.invoke`。
- 判不通过但 issues 为空 → 按通过处理（避免空修复轮）。
...
- 修复稿先走 regex 快筛；**通过后仍然跑一次 LLM 深审**（`review_llm_only()`），双通过才定稿——
  旧版"regex 过即定稿"，修复稿从未被 LLM 看过，改写质量问题无人发现。LLM 判不通过则记
  `reviewer_repair_llm_rejected` 并按修复稿定稿（单轮修复不重试，避免 ping-pong）。
- regex 仍报问题时同样调用 `review_llm_only()`：LLM 判通过 → 记 `reviewer_regex_likely_false_positive`
  （regex 疑似误报）后按修复稿定稿。旧版此处调 `review()`，被正则短路，LLM 实际从未被调用
  （日志却写"触发 LLM 深审"）。
- **修复前后事实比对**：比对"原稿提到、修复稿丢掉、且 `compact_facts` 里确实存在"的十神/神煞
  （名表 `SHISHEN_NAMES`/`SHENSHA_NAMES` 单一事实源），命中即记 `repair_dropped_facts` + WARNING。
  只观测、不触发新一轮修复——用于发现"修复把正确信息一起删掉"（实测恋爱轮修复稿删掉了正确的
  流年神煞"红艳煞"）。
...
| `REVIEWER_SYSTEM` | LLM 深审提示（12 维度） | 2359 |
