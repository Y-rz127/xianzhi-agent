...
2. **大运干支**：`(干支)(大运|运)` 必须落在本盘大运集合里（流年干支也算合法，避免"己酉年"写成
   "己酉运"这类近义表达误杀）；编造大运直接记问题。
...
**修复后的二次判定**：修复稿先走 regex 快筛，通过即定稿（省 1 次调用）；regex 仍报问题时调用
`review_llm_only()` 只跑 LLM 深审——旧版此处调 `review()`，而 `review()` 会先跑 regex 并立刻短路，
LLM 实际从未被调用（日志却写"触发 LLM 深审"）。LLM 判通过时记
`reviewer_regex_likely_false_positive` 并按修复稿定稿。
