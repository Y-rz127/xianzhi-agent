...
│                                                                             │
│   ┌──────────┐    普通边      ┌──────────┐    普通边      ┌──────────┐      │
│   │ classify │───────────────▶│  chart   │───────────────▶│ retrieve │      │
│   │ 分类节点  │               │ 扩盘节点  │               │ 检索节点  │      │
│   └──────────┘               └──────────┘               └──────────┘      │
│        │                         ▲                            │            │
│        │ 输入: user_prompt       │ 输入: chart_context        │ 输入: intent│
│        │ 输出: intent, worker    │ 输出: chart_context        │ 输出: knowledge│
│        │                         │ 扩展流年范围               │ 闲聊短路跳过 │
│        │                         │                            │            │
│        │                    ┌────┴────┐              ┌───────┴───────┐    │
│        │                    │ 条件边   │              │    普通边     │    │
...
│        │                    ▼                                               │
│        │              ┌──────────┐                                         │
│        │              │checkpointer│ ← 每节点执行完自动快照 State             │
│        │              │ 检查点持久化│   支持断点续跑/暂停恢复/状态回溯          │
│        │              └──────────┘                                         │
└─────────────────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────────────────┐
│                         子图嵌套 (Subgraph) 概念演示                          │
│              一个完整的 Graph 可以作为另一个 Graph 的节点                      │
└─────────────────────────────────────────────────────────────────────────────┘
   ...
   实际项目目前是单层 Graph（6 节点），但代码结构已为子图嵌套预留了接口：
   - xianzhi_langgraph.py 的 create_xianzhi_graph() 返回一个 compiled graph
   - 这个 graph 可以作为一个节点被更大的 Supervisor Graph 调用
   - 如果要扩展为"多 Agent 协作"，每个 Worker 可以是一个子图

┌─────────────────────────────────────────────────────────────────────────────┐
│                    Human-in-the-Loop 人类介入演示                             │
│                 在每个节点执行前，可设置中断等待人工审批                          │
└─────────────────────────────────────────────────────────────────────────────┘

   ┌──────────┐     ┌──────────┐     ┌──────────────────────┐
   │ generate │────▶│  check   │────▶│   ⏸ 中断点 (interrupt) │
   │ 生成节点  │     │ 校验节点  │     │  "发现 3 个问题，是否    │
   └──────────┘     └──────────┘     │   批准修复？"           │
                                     │                      │
                                     │  人工操作：            │
                                     │  ┌────────────────┐  │
   ...
   实际项目中暂未启用 Human-in-the-Loop（全自动流水线），
   但 LangGraph 框架原生支持，只需在 graph.compile(checkpointer=...) 后
   调用 graph.interrupt() 即可在指定节点暂停。
...
└─────────────────────────────────────────────────────────────────────────────┘
              ...
              ▼                   ▼                   ▼
    ┌─────────────────┐ ┌─────────────────┐ ┌─────────────────┐
    │   有向图编排       │ │  Checkpointer    │ │  Human-in-Loop  │
    │                  │ │                  │ │                 │
    │ 节点(Node)       │ │ 每节点执行后      │ │ 可在任意节点     │
    │ 边(Edge)         │ │ 自动快照 State    │ │ 设置 interrupt   │
    │ 条件边(Condition)│ │ 持久化到存储       │ │ 暂停等待人工      │
    │ 入口/出口         │ │ 断点续跑/回溯     │ │ 修改State后继续   │
    └─────────────────┘ └─────────────────┘ └─────────────────┘
              │
              │ 子图嵌套
              ▼
    ┌─────────────────┐
    │  完整的 Graph    │  ← 可以作为另一个 Graph 的节点
    │  作为子图嵌入     │     实现模块化分层架构
    └─────────────────┘
```
                    ┌─────────────┐
...
用户输入
    ...
    ▼
┌─────────────────────────────────┐
│ 第 0 层：前置拦截（_looks_off_topic）│
│ 条件：文本 > 100 字 + 零命理信号词   │
│ 命中 → 直接标记 chitchat，跳过 LLM   │
└──────────────┬──────────────────┘
               │ 未命中
               ▼
┌─────────────────────────────────┐
│ 第 1 层：关键词短路（detect_domain） │
│ 条件：命中"你好""在吗"等闲聊词       │
│ 命中 → 直接标记 chitchat，跳过 LLM   │
└──────────────┬──────────────────┘
               │ 未命中
               ▼
┌─────────────────────────────────┐
│ 第 2 层：LLM 拆解（_decompose_query）│
│ prompt 已告知：诗歌/故事/流水账 →    │
│ chitchat + queries=[]             │
│ 命中 → chitchat Worker 闲聊回复     │
└──────────────┬──────────────────┘
               │ LLM 失败
               ▼
         classify_question (关键词兜底)
### 三种角色
...
| **古籍真实性** | 正则层：比对 `《XXX》原文：` 标注是否在检索结果或古籍白名单中出现；LLM 深审层：识别"凭空出现、检索未提供的古文引用"（正则只能查书名，LLM 才能查伪造的古文句子） | 检索结果无《XX书》，回答却引用了；或回答里出现检索未提供的《滴天髓》伪句 |
| **合规校验** | 扫描死期/赌博/符咒/堕胎等红线关键词 | 回答含"死期""买彩票必赢" |

> LLM 深审（`_REVIEWER_SYSTEM`）的 6 个维度：逻辑自洽、断法准确性、知识一致性、**古籍真实性**、表达质量、事实复查。其中"古籍真实性"同时由正则快筛（查书名白名单）与 LLM 深审（查伪造古文句子）双重覆盖，真正闭合文档宣称的"事实 + 古籍真实性 + 合规"三重校验。事实性硬错误（干支错误、身强身弱与推论矛盾、伪造古籍）零容忍，直接判不通过。
